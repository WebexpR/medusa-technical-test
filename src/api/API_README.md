...
Medusa supports adding custom API Routes using a **file based approach**. 

This means that you can add files in the `/api` directory and the files path will be used as the API Route path. For example, if you add a file called `/api/store/custom/route.ts` it will be available on the `/store/custom` API Route.
...
To resolve a service from the container, you can use the `resolve` method on the container. The `resolve` method takes a service identifier as an argument and returns the service instance.
...
The service identifier is the name of the service that you want to resolve **IN CAMEL CASE**.

So for example, if you want to resolve the `ProductService`, the service identifier is `productService`.
Another example, if you want to resolve the `OrderService`, the service identifier is `orderService`.
Or `WishlistService`, the service identifier is `wishlistService`.
